...
mko_clean %>%
  mutate(month_name = factor(month_name, levels = month.name)) %>% # Conv month to factor and order using built in function
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_histogram(position = 'identity', alpha = 0.5) # Sit all bars right on the x-xis

# Warning to default to 30 bins and consider changing bin width. Also missing some data
# Months not in order, so will change now
```
...
mko_clean |> 
  ...
  ggplot(aes(x = Temp_bot)) + # Remove color to limit distractions
  geom_histogram() +
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) |> 
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_density(alpha = 0.5, adjust = 0.5) + # taking a smaller range of values, averaging it
  scale_fill_manual(values = c("#2C5374", "#ADD8E6", "#8B3A3A") )
```
...
ggplot(dummy_data, aes(x = value, fill = group)) +
  geom_histogram(position = "identity", alpha = 0.7, binwidth = 3) + # changing binwidth makes it useless
  geom_rug(aes(color = group), alpha = 0.75) # See difference in sample size

# density plot ----
ggplot(dummy_data, aes(x = value, fill = group)) +
  geom_density(alpha = 0.7) +
  geom_rug(aes(color = group), alpha = 0.75) # Group A is almost as tall as group b...may be inclined to believe A has same sample size as B, but that is not true.
```
...
ggplot(mko_clean, aes(x = Temp_bot, y = month_name, fill = after_stat(x))) + # Compute gradient based on x variabel
  geom_density_ridges_gradient(rel_min_height = 0.01,
                               scale = 3) +
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A")) + # Does not apply automatically, needs after_stat function
  scale_y_discrete(limits = rev(month.name)) # Adjust axis of y variable

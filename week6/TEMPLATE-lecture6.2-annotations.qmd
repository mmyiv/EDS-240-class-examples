...
lob_plot +
  ...
  ) + # Fix the fill of the bbox, although how does the black outline get thinner compared to the previous code chunk? Is it due to annotate overriding it?
  annotate(
    geom = "curve",
    x = 0.3, xend = 0.5,
    y = 23.8, yend = 19,
...
```{r}
# Add Annotations to base map
ggplot(data = mono, aes(x = year, y = lake_level)) +
  annotate(
    geom = "rect",
    xmin = 1941, xmax = 1983,
    ymin = 6350, ymax = 6440,
    fill = "gray90") + # Highlight area of unrestricted diversions by graying out a section
  annotate(
    geom = "text", 
    x = 1962, y = 6425,
    label = "unrestricted diversions\n(1941 - 1983)",
    size = 3) + # Label for grayed out section
  geom_line() + # add this last so it is not covered by grey out part
  labs(x = "\nYear",
       y = "Lake surface level\n(feet above sea level)\n",
       title = "Mono Lake levels (1850 - 2017)\n",
       caption = "Data: Mono Basin Clearinghouse") +
  ...
  annotate(
    ...
    size = 3) + # Final shrimp annotation
  theme_light() +
  theme(
    plot.title.position = "plot",
    plot.title = element_text(size = 16),

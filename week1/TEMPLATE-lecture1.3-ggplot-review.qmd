...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  geom_point(color = 'blue') # putting in aes will make it salmon bc it is making legend...
```
...
ggplot() +
  ...
  geom_point(data = penguins_summary, 
             mapping = aes(x = mean_bill_length_mm,
                           ...
                           fill = species), # go outside aes to update size and shapes. 
...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm)) + 
  geom_point(aes(color = species)) + # just want color to apply to points, not geom_smooth
  geom_smooth(method = 'lm')
```
...
ggplot(penguin_summary, aes(x = species, y = n)) + # n is name of count column 
  geom_bar(stat = 'identity') # bar all the same height... default stat is counting the # of rows
```
...
#| warning: false
ggplot(penguins, aes(x = species, fill = island)) +
  geom_bar(position = 'fill') # makes all same height
```
...
#| warning: false
ggplot(penguins, aes(x = species, fill = island)) +
  geom_bar(position = 'dodge') # places all bars on same x axis
```
...
#| warning: false
ggplot(penguins, aes(x = species, fill = island)) +
  geom_bar(position = position_dodge2(preserve = 'single')) # makes all bars same width
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar() +
  coord_cartesian() # this is default and does not need to be included
```
...
ggplot(penguins, aes(x = flipper_length_mm, fill = species)) +
  ...
  facet_wrap(~species, ncol = 1) # set single column of facets, easier to compare dist
```
